...
load_model = True
if load_model == True:
    model_checkpoint = torch.load('model_pretrained.pth', map_location=device)
...
def plot(frame_idx,rewards, losses):
    ...
    y2 = []
    print(losses)
    for x in range(frame_idx):
        x1.append(x)
    for x in rewards:
        y1.append[x]
    for x in losses:
        y2.append[x]
    plt.title('frame %s. reward: %s' % (frame_idx, max(y1)))
    plt.plot(x1, y1)
    plt.subplot(132)
    plt.title('loss')
...
for frame_idx in range(1, num_frames + 1):
    ...
    if frame_idx % 10000 == 0:
        ...
        torch.save(model.state_dict(), "model_pretrained3.pth")


    data_save = {}
    data_save['loss'] = losses
    data_save['reward'] = all_rewards
plot(frame_idx, data_save['reward'], data_save['loss'])
